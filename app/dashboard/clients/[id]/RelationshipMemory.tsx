...
export async function RelationshipMemory({ clientId }: { clientId: string }) {
  const supabase = await createClient();

  const { data: objects } = await supabase
    .from("intelligence_objects")
    .select("*, meetings(created_at), contacts(full_name)")
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="font-mono text-xs text-ink-muted uppercase tracking-widest">Relationship memory</p>
      </div>
      <div className="space-y-2">
        {objects.map((o: any) => {
          const s = STATUS_STYLE[o.temporal_status] ?? STATUS_STYLE.confirmed;
          const Icon = s.icon;
          const date = o.meetings?.created_at ? new Date(o.meetings.created_at).toLocaleDateString() : "";
          return (
            <div key={o.id} className={`border rounded-lg p-4 ${s.cls.replace(/text-\S+/, "")}`}>
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                ...
                {o.contacts?.full_name && (
                  <span className="text-xs text-ink-muted">· {o.contacts.full_name}</span>
                )}
                <span className="font-mono text-[10px] text-ink-muted ml-auto">{date}</span>
              </div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                {o.meeting_id && (
                  <Link href={`/dashboard/meetings/${o.meeting_id}`} className="text-[10px] text-teal hover:underline">
                    View meeting →
                  </Link>
                )}
              </div>
