...
export function CrmUpdate({ meetingId }: { meetingId: string }) {
  ...
  async function run() {
    setLoading(true);
    setError("");
    const res = await fetch("/api/update-crm", {
      method: "POST",
    ...
    <section className="bg-surface border border-border rounded-xl p-6 card-shadow">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-mono text-xs text-teal uppercase tracking-widest">Update client record</p>
        </div>
      ...
      {!update && !loading && !error && (
        <p className="text-xs text-ink-muted">Sets the sales stage, next action, and current risk on the client record based on this meeting.</p>
      )}

      {update && (
        <div className="space-y-2">
          <div className="bg-paper border border-border rounded-lg px-4 py-3">
            <p className="text-xs text-ink-muted">Sales stage</p>
            <p className="text-ink font-medium">
              {update.sales_stage}
              {update.stage_changed && <span className="text-xs text-good ml-2">changed</span>}
            </p>
            <p className="text-xs text-ink-muted mt-1">{update.rationale}</p>
          </div>
