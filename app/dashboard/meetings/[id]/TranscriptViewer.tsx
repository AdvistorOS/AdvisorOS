...
export function TranscriptViewer({ meetingId, attendeeNames, attendeeContactIds, clientId }: {
  meetingId: string; attendeeNames: Record<string, string>; attendeeContactIds: Record<string, string>; clientId: string;
}) {
    ...
    <section className="bg-surface border border-border rounded-xl p-6 card-shadow">
      ...
      <div className="space-y-2.5">
        {visible.map((u: any, i: number) => {
          const contactId = attendeeContactIds[u.speaker];
          const name = attendeeNames[u.speaker];
          return (
            <div key={i} className="flex gap-3">
              ...
              <div>
                <span className="mr-1">
                  <SpeakerBadge speakerLabel={u.speaker} clientId={clientId} attendee={contactId ? { contactId, name } : undefined} />
                </span>
